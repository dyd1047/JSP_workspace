...
<%
	...
	//따라서 IO, 네트워크등의 처리를 해야 하는데, 이 차제만으로도 하나의 개발 프로젝트일 것이다.
	//해결책? 누군가가 만든 라이브러리를 이용해서 개발시간을 단축하자!
	//현재 우리가 선택한 라이브러리는 cos.jar라는 Oreilly 라는 출판사에서 제작한 컴포넌트이다!
	String saveDirectory="D:/User/yong/LN/workspace/athome/workspace-HTML/javaee_workspace/BoardApp/WebContent/data"; //하드디스크의 물리적 full 경로를 명시해야 한다.
	int maxSize = 2*1024*1024; //2M byte
	String encoding = "utf-8";
	
	//FileRenamePolicy policy : 업로드시, 동일한 파일을 업로드했을 때? 자동으로 이름을 부여한다.
	//파일명은 개발자가 주도하여 명명하므로, policy를 굳이 이용할 필요 없다..
	try{
		...
		//파일명을 개발자가 정한 규칙으로 변경해야 한다.. 현재 시간의 밀리세컨드까지 구해보자
		long time = System.currentTimeMillis();
		//구한 시간에 확장자를 붙이면 최종적으로 파일명이 된다.
		out.print(time);
		...
		//File 클래스의 .api문서를 찾아서, 파일명을 바꾸는 메서드를 찾자
		//스트림과 상관없고, 파일을 제어하는 거라서 파일 클래스의 메서드를 찾기
