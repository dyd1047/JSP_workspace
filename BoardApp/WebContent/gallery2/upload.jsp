...
<%
	//파일 업로드 컴포넌트의 종류에는 여러가지가 있지만, 그 중 아파치의 공식 업로드 컴포넌트를 사용해본다.
	
	String saveDir = "D:/User/yong/LN/workspace/athome/workspace-HTML/javaee_workspace/BoardApp/WebContent/data";
	int maxSize = 2*1024*1024; //2M byte제한
	//업로드 객체를 생성해주는 팩토리 객체
	DefaultFileItemFactory itemFactory = new DefaultFileItemFactory();
	itemFactory.setRepository(new File(saveDir)); //저장될 위치
	itemFactory.setSizeThreshold(maxSize); //용량 제한

	//이 객체가 실제 업로드를 수행함
	ServletFileUpload upload = new ServletFileUpload(itemFactory); //설정정보를 생성자의 인수로 전달!
	
	//FileItem은 클라이언트의 전송 정보 하나하나를 의미한다. 즉 html에서의 input 텍스트박스, file 컴포넌트 등...
	...
	for(FileItem item : items){
		//반복문으로 처리되다 보니, 파일만 따로 처리를 하려면, 구분 로직이 필요함..
		out.print(item.getFieldName()+"의 값은 "+item.getString());
		...
		if(item.isFormField()){ //텍스트박스가 아닌것만 업로드 처리
			...
			out.print("저장된 파일크기 : "+item.getSize()+" bytes <br>");
		}
	}
%>
