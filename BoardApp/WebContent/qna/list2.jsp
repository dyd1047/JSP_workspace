...
<%
	...
	//아래의 코드는 아무때나 하는게 아니다. 즉 누군가가 파라미터를 넘겼을때만.. 즉 페이지 넘버를 클릭한 경우를 전제로
	//숫자화 시키는 코드이다.
	if(request.getParameter("currentPage")!=null){ //즉 페이지를 넘겼다면.. 넘버를 클릭했다면...
...
<table>
  <tr>
	...
	<%for(int i = 1; i <= pageSize; i++){ %>
  	<%if(num<1) break;%>
  	<%
  		//브레이크문을 만나지 않았다는 것은 레코드가 있다는 것이므로, break문 아래에서 데이터를 추출하자
  ...
  <tr>
	<td colspan="5" style="text-align:center">
		<%if(firstPage-1>0){ %>
		<a href="/qna/list2.jsp?currentPage=<%=firstPage-1%>">[◀prev]</a>
		<%}else{ %>
		<a href="javascript:alert('처음 페이지 입니다.')">[◀prev]</a>
		<%} %>
		<%for(int i = firstPage; i <= lastPage; i++) {%>
		<%if(i>totalPage)break; //페이지를 출력하는 i가 총 페이지수에 도달하면 반복문 빠져나와라...%>
		<a href="/qna/list2.jsp?currentPage=<%=i%>" <%if(currentPage==i){ %>class="pageNum"<%} %>>[<%=i %>]</a>
		<%} %>
		<%if(lastPage+1<totalPage){ %>
		<a href="/qna/list2.jsp?currentPage=<%=lastPage+1%>">[next▶]</a>
		<%}else{ %>
		<a href="javascript:alert('마지막 페이지 입니다.')">[next▶]</a>
		<%} %>
	</td>
	</tr> 
	<tr>
